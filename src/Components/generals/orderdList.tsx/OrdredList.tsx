interface OrdreListProps {
  ...
  export const OrdreList = ({displ}:OrdreListProps) => {
    return (
      <div className={` h-fit flex w-full justify-center  ${displ?.classNames ? displ?.classNames : ""}
      my-5
      items-center
      flex-wrap
      relative z-[5] px-20
      ...
      bg-fixed `}>
        ...
        <div className='py-10 px-5 w-1/2 h-full  flex flex-col   gap-9 max-md:w-[100%]'>
          <div className=' w-full  flex flex-col gap-2'>
            <span className=' text-secondary text-5xl font-bold  max-md:text-5xl
            
            whitespace-nowrap '> {displ?.title}</span>
            <span className='font-play-fair text-4xl text-white' > {displ?.subtitle} </span>
          </div>
          
          {
            displ?.text && displ?.text?.map((text:{title?:string| JSX.Element,text?:string| JSX.Element} ,i:number)=>(
      <div key={i} className='mt-5 flex flex-wrap  gap-[20px] '>
        ...
        {displ.sideImg && 
        <div className="w-1/2 max-md:w-[100%]  mt-auto  flex  justify-center"><img src={displ?.sideImg} className=' h-[40rem]' alt="" /></div>}
      </div>
    )
  }